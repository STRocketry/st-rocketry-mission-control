...
export const ConfigurableCommandPanel = ({ onSendCommand, isConnected }: ConfigurableCommandPanelProps) => {
  const [button1Config, setButton1Config] = useState<ButtonConfig>({ ...defaultButtonConfig });
  ...
  const handleSaveConfig = () => {
    ...
    if (editingButton === 1) {
      setButton1Config({ ...tempConfig });
    } else if (editingButton === 2) {
      setButton2Config({ ...tempConfig });
    }
  ...
  const renderButton = (config: ButtonConfig, buttonNumber: 1 | 2) => (
    <div key={buttonNumber} className="flex gap-2">
      <Button
        onClick={() => handleButtonClick(config)}
        disabled={!isConnected}
        className="flex-1 h-12 text-sm font-bold transition-all duration-200"
        style={{ 
      ...
      <Button
        variant="outline"
        size="icon"
        onClick={() => handleConfigureButton(buttonNumber)}
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-4 shadow-sm">
      <h3 className="text-base lg:text-lg font-bold mb-3 text-card-foreground">CONFIGURABLE COMMANDS</h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setTempConfig({ ...tempConfig, command: e.target.value })}
            ...
            <div>
              ...
              <Input
                ...
                min={1}
                max={50}
                value={tempConfig.numberOfCommands}
                onChange={(e) => setTempConfig({ ...tempConfig, numberOfCommands: parseInt(e.target.value) || 1 })}
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                type="number"
                min={1}
                value={tempConfig.intervalMs}
                onChange={(e) => setTempConfig({ ...tempConfig, intervalMs: parseInt(e.target.value) || 1 })}
              />
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                {predefinedColors.map((color) => (
                  <button
                    key={color.value}
                    className={`h-8 rounded border-2 ${tempConfig.color === color.value ? 'border-primary' : 'border-border'}`}
                    style={{ backgroundColor: color.value }}
                    onClick={() => setTempConfig({ ...tempConfig, color: color.value })}
                    title={color.name}
                  />
                ))}
              </div>
